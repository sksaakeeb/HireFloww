...
const InterviewDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Fetch Interviews & Job Info in parallel
        const [intRes, jobRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_BASE_URL}/interviews/${jobId}`, { withCredentials: true }),
      //     axios.get(`${import.meta.env.VITE_API_BASE_URL}/jobs/${jobId}`, { withCredentials: true })
        ]);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_BASE_URL}/interviews/${id}`, { withCredentials: true });
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-4xl font-black text-gray-800 tracking-tight">
                  {job?.companyName} <span className="text-indigo-600">Journey</span>
                </h1>
                <div className="flex items-center gap-3 text-sm text-gray-500 font-medium mt-1">
                  <span className="flex items-center gap-1"><Clock size={14}/> {job?.jobRole}</span>
                  <span>•</span>
          ...
          <button 
            ...
            className="flex items-center gap-2 px-8 py-4 bg-gray-900 text-white rounded-2xl font-black shadow-xl hover:bg-black transition-all active:scale-95 text-xs tracking-widest uppercase"
          >
        ...
        {interviews.length > 0 ? (
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] p-8 rounded-[3rem] bg-white/40 backdrop-blur-2xl border border-white/60 shadow-xl hover:shadow-2xl transition-all duration-500">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <time className="text-[10px] font-black text-gray-400 uppercase flex items-center gap-1">
                      <Calendar size={12} /> {new Date(round.date).toLocaleDateString("en-GB")}
                    </time>
                  </div>

                  <div className="bg-white/60 rounded-[2rem] p-6 border border-white/50 shadow-inner mb-6">
                    <p className="text-sm text-gray-600 leading-relaxed font-medium italic">
                      {round.feedback ? `"${round.feedback}"` : "No specific round notes recorded."}
                    </p>
                  </div>

                  <div className="flex items-center justify-between pt-4 border-t border-white/40">
                    {round.assignment ? (
                      <a href={round.assignment} target="_blank" rel="noreferrer" className="text-[10px] font-black text-emerald-600 flex items-center gap-1 hover:underline">
                    ...
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="p-3 bg-white text-amber-500 rounded-xl border border-amber-100 hover:bg-amber-500 hover:text-white transition-all shadow-sm"
                      >
                        <Pencil size={16} />
                      </button>
                      <ConfirmDelete onConfirm={() => handleDelete(round._id)}>
                        <button className="p-3 bg-rose-50 text-rose-500 rounded-xl border border-rose-100 hover:bg-rose-500 hover:text-white transition-all shadow-sm">
                          <Trash2 size={16} />
                        </button>
                      </ConfirmDelete>
                    </div>
          ...
          <div className="text-center py-24 bg-white/20 backdrop-blur-md rounded-[4rem] border-2 border-dashed border-white">
            ...
            <p className="text-gray-400 font-black uppercase tracking-widest text-xs">No rounds recorded yet</p>
          </div>
        )}
      </div>
    </div>
